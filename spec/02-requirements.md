...
- `TWITTER_AUTH_TOKEN` の多段階自動読み込み（環境変数最優先）
- n8n 向け REST API
...
| ID | 要件 |
|----|------|
| FR-01-1 | 起動時に複数ソースから `TWITTER_AUTH_TOKEN` を **自動読み込み** する |
| FR-01-2 | 解決順位: **`process.env` 最優先** → `DOTENV_PATH` または `~/.config/x-trends/.env` → `cwd/.env` → パッケージルート `.env`（開発用） |
...
| ID | 要件 |
|----|------|
| FR-08-1 | 設定値を優先順位付きで `dotenv` 自動読み込み（`process.env` が常に最優先） |
...
| ID | 要件 |
|----|------|
| NFR-03-1 | `TWITTER_AUTH_TOKEN` は `.env` または環境変数で管理（git 除外） |
...
1. 起動時に `.env` 候補を優先度の低い順に読み込み、最後に起動前の `process.env` を復元する（**環境変数が常に最優先**）
2. CI / グローバル CLI では `TWITTER_AUTH_TOKEN` を `process.env` に注入するのが推奨
3. ローカル開発ではリポジトリルートの `.env` または `~/.config/x-trends/.env` を使用

**`TWITTER_AUTH_TOKEN` 解決順位（高→低）:**

| 順位 | ソース | 用途 |
|------|--------|------|
| 1 | `process.env.TWITTER_AUTH_TOKEN` | シェル export、CI Secrets、インライン指定 |
| 2 | `DOTENV_PATH` または `~/.config/x-trends/.env` | グローバルインストール・ユーザー設定 |
| 3 | `process.cwd()/.env` | プロジェクトローカル設定 |
| 4 | パッケージルート `.env` | リポジトリ開発時のフォールバック |

### 6.2 変数一覧

| 変数名 | 必須 | デフォルト | 説明 |
|--------|------|-----------|------|
| `TWITTER_AUTH_TOKEN` | はい | — | X `auth_token`（環境変数または `.env`） |
...
1. `TWITTER_AUTH_TOKEN`（環境変数または `.env`）のみで起動可能
